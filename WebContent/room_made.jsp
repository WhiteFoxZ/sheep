...
if(userinfo!=null){
    ...
    //계좌번호 LOV
    EmsHashtable[] accuntLov = dbm.selectMultipleRecord("SELECT CD_ID,CD_MEANING FROM comm_info INFO where INFO.CD_GROUP_ID='ACCUNT_NO' AND INFO.LOGIN_ID=?",
    		new String[] { LOGINID });
	...
	String ACCONT_NO = request.getAttribute("ACCONT_NO")!=null?request.getAttribute("ACCONT_NO").toString():""; // 카드ID
...
sb.append("SELECT  max(case when CD_ID ='CAR_ADD_CNT' then PRICE else 0 end) CAR_ADD_CNT \n");
sb.append(",max(case when CD_ID ='CAR_ADD_CNT' then NULLIF( EXT1,0)  else 0 end )CAR_CNT     \n");
...
sb.append(" AND LOGIN_ID=?                                                               \n");
    ...
    String CAR_ADD_CNT = hash_option[0].getString("CAR_ADD_CNT");
    
    int CAR_CNT = Integer.parseInt( hash_option[0].getString("CAR_CNT") );
...
<title>예약등록</title>
        ...
        var submitFlag=true;
            ...
            function save(event){
            	...
            	if(document.frmMain.USER_EMAIL.value==""){
            		showMsg('메일주소를 넣어주세요.');            		            		
            		return false;
            		...
            		if(!emailCheck(document.frmMain.USER_EMAIL.value)){
            			showMsg('email 을 정확히 넣어주세요.');  
            			return false;
            		};
            	...
            	if(document.frmMain.CAR_NUM.value==""){
            		showMsg('차량번호를 입력해주세요.');
            		return false;
            	} 
        ...
        function cal(){
        	...
        	var car_cnt = $('select[name="CAR_ADD_CNT"]').val();
        	
        	var peo_cnt = $('select[name="PEOPLE_ADD_CNT"]').val();
        	
        	var car_unit = <%=CAR_ADD_CNT%>;
        	 
        	var peo_unit = <%=PEOPLE_ADD_CNT%>;
        	 
    		 
    		var TOTAL_PAY = MAIN_PRICE + HOT_PRICE + (car_cnt*car_unit) +  (peo_cnt*peo_unit);
         	...
         	if(hot_day_cnt>0){
         		...
         		if(car_cnt>0)
         		html=html+"+추가차량:"+car_cnt+"대["+(car_cnt*car_unit)+"]";
        ...
        $(document).ready(function(){
        	...
        	$('#LI_CAR_CNT').hide(); 
        	 ...
        	 $('select[name="CAR_ADD_CNT"]').change(function(){        		
        		 cal();
       		});
...
<ul data-role="listview" data-inset="true">
              ...
              <li data-role="fieldcontain">
                           ...
                           <input type="text" name="USER_EMAIL" id="USER_EMAIL" value="<%=USER_EMAIL %>" placeholder="Mail을 입력하세요." required >                                                                             
              ...
              <li data-role="fieldcontain">
                           ...
                           <label for="CAR_NUM"><H4>차량번호:</H4> </label> 							
                           <input type="text" name="CAR_NUM" id="CAR_NUM" value="<%=CAR_NUM %>" placeholder="차량번호 입력하세요." data-enhance="true">                                                    
              </li>   
                        
              <li data-role="fieldcontain">

                           <label for="ACCONT_NO"><H4>계좌: </H4></label> 

                           <select name="ACCONT_NO" id="ACCONT_NO" data-icon="info" data-iconpos="left" data-native-menu="false">

                                        <option value="">선택하세요</option>
                                        
                                        <%=EmsOption.getOption(accuntLov,"CD_ID","CD_MEANING","") %>
                                        
                           </select>                                                      
              </li>
                                         
              <li data-role="fieldcontain" id="LI_CAR_CNT">

                           <label for="CAR_ADD_CNT"><H4>추가차량: </H4></label> 

                           <select name="CAR_ADD_CNT" id="CAR_ADD_CNT" data-icon="info" data-iconpos="left" data-native-menu="false">
...
%>
              ...
              <li data-role="fieldcontain">
                ...
                <li data-role="collapsible" class="jqm-demos jqm-content">

<h2>개인정보 약관</h2>
   
     <label for="info">개인정보 동의<input type="checkbox" name="chk1" id="chk1" ></label>
                 
                 
                 <textarea id="info" cols="40" rows="1" name="info">
 개인정보 취급방침
1. 수집하는 개인정보(캠핑장 예약에 필요한 최소한의 정보를 수집합니다.)
1)성명
2)이메일
3)휴대폰번호(예약확인및취소)
4)캠핑장 예약정보
...
2. 개인정보의 보유및 이용기간
 - 예약에 관한 개인정보는 캠핑장 예약에 필요한 개인정보만을 관리합니다.(예약일 기준1달)
 - 예약에 관한 모든 개인정보는 예약일 기준 1달까지만 보유하면 이후 DB에서 백업없이 모두 자동삭제 되어 파기됩니다.
