...
import com.ems.action.board.CaptchaServiceSingleton;
...
public class RoomMadeAction {
	...
	public void modify() {
		...
		String USER_EMAIL = request.getParameter("USER_EMAIL"); // 카드ID
		String ACCONT_NO = request.getParameter("ACCONT_NO"); // 카드ID
		String USER_NAME = request.getParameter("USER_NAME"); // 카드ID
		String DAY = request.getParameter("DAY"); // 일자
		String CAR_NUM = request.getParameter("CAR_NUM"); // 일자
		String CAR_ADD_CNT = request.getParameter("CAR_ADD_CNT");
		...
		if(!validate()){
			...
			request.setAttribute("ACCONT_NO", ACCONT_NO);
			request.setAttribute("USER_NAME", USER_NAME);
			request.setAttribute("CAR_NUM", CAR_NUM);
			request.setAttribute("CAR_ADD_CNT", CAR_ADD_CNT);
		...
		try {
			 ...
			 if(data.length>0){
				...
				new GMailSender().mailSender(subject, to, content);
